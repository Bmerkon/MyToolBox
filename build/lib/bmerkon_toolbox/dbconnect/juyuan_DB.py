...
class JuYuanDB:
    def __init__(self):
        ...
        self.sql_fpath = os.path.dirname(os.path.abspath(__file__))+'/juyuan_sql'
    ...
    def data2mat(self,innercode_list,first_day,last_day,df,value_name,ffill=False):
        # 计算日期
        tradingdate = self.query_tradingday(first_day, last_day)
    ...
    def process_data(self,innercode_list,first_day,last_day,df,value_name,ffill=False,mode='mat'):
        if mode == 'mat':
            data_mat = self.data2mat(innercode_list,first_day,last_day,df,value_name,ffill=ffill)
            return data_mat
        else:
            return df
